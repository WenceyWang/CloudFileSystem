		...
		: ProgramBase <Program , ProgramExitCode , ProgramSetting , ProgramSettingCatalog>
	{
		...
		public override async void Start ( string [ ] args )
		{
			...
			if ( string . IsNullOrWhiteSpace ( Setting . ClientId )
			  ...
			  || string . IsNullOrWhiteSpace ( Setting . SqlConnectionString ) )
			{
				Logger . LogCritical (
			...
			else
			{
				try
				{
					...
					ex . Message . Contains ( "AADSTS65001" ) )
				{
					...
					{
						Logger . LogCritical (
											  "AADSTS50126: Invalid username or password, check setting file to fix that." ) ;
					}
				...
				catch ( MsalClientException ex ) when ( ex . ErrorCode
		...
		{
			builder . AddFilter ( level => true ) . AddDebug ( ) . AddConsole ( ) ;
		}
